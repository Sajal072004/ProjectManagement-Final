...
const HomePage = () => {
  ...
  const {
    ...
    isError: tasksError,
  } = useGetTasksByCognitoIdQuery({cognitoId});
  
  const { data: projects, isLoading: isProjectsLoading, isError: projectsError } =
    useGetProjectsQuery({ cognitoId }); // Use cognitoId here
  ...
  // If either tasks or projects are loading, show a loading message
  if (tasksLoading || isProjectsLoading) return <div>Loading..</div>;

  // If there is an error fetching tasks or projects, show an error message
  if (tasksError || projectsError) return <div>Error fetching data</div>;
